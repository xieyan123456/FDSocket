...
- (void)allMessage:(NSArray*)msgArray isAtLastRow:(BOOL) isAtLastRow {
    ...
    for(id msgDic in msgArray) {
        ...
        if ([[msgDic objectForKey:@"issend"] integerValue] == 0) {
            ...
            [failed addGestureRecognizer:[UITapGestureRecognizer bk_recognizerWithHandler:^(UIGestureRecognizer * _Nonnull sender, UIGestureRecognizerState state, CGPoint location) {
                ...
                for (NSMutableDictionary *dic in [self.IndicatorDic allValues]) {
                    if ([[dic allValues] indexOfObject:failed]!= NSNotFound) {
...
- (void)setToolBar:(BOOL)isShowTextfile {
    ...
    [_moreBut bk_addEventHandler:^(id  _Nonnull sender) {
    ...
    [_emotions bk_addEventHandler:^(id  _Nonnull sender) {
    ...
    [_sendBtn bk_addEventHandler:^(id  _Nonnull sender) {
...
- (UIImageView*)picture:(NSString*)msg and:(BOOL)isfromself {
    ...
    if([msg rangeOfString:@"://"].length>0){
        [image setImageWithURL:[NSURL URLWithString:msg] placeholderImage:nil usingActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    }else{
        image.image = [imageProcess getUIImageByName:msg];
    }

    image.userInteractionEnabled = YES;
    [image addGestureRecognizer:[UITapGestureRecognizer bk_recognizerWithHandler:^(UIGestureRecognizer * _Nonnull sender, UIGestureRecognizerState state, CGPoint location) {
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == 0) {
        ...
        if([dic[@"type"] isEqualToString:@"image"]){
            ...
            [upload postRequestWithURL:[httpUrl stringByAppendingString:@"/upload/img"] postParems:nil image:image picFileName:[NSString stringWithFormat:@"%lld",[Util getTimeUtil1970]] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
                
                if (error) {
                    NSLog(@"============Error:%@",error.localizedDescription);
                }else {
                    NSString* result= [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                    NSNumber *code = [[Util StringOrDic2NSDic:result] objectForKey:@"code"];
                    if (code.intValue == 0) {
                        Message *message = [MessageManager getBufQSTBody:result type:@"image" packNumber:markid isLeave:_isLeave];
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (UIImage *img in photos) {
            imageProcess *upload = [[imageProcess alloc]init];
            [upload postRequestWithURL:[httpUrl stringByAppendingString:@"/upload/img"] postParems:nil image:img picFileName:[NSString stringWithFormat:@"%lld",[Util getTimeUtil1970]] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
                ...
                if (error) {
                    ...
                    NSNumber *code = [[Util StringOrDic2NSDic:result] objectForKey:@"code"];
                    if (code.intValue == 0) {
                        [self uploadPicCallback:result upImage:img];
                    }else {
                        [self uploadPicCallback:nil upImage:img];
                    }
                }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        imageProcess *upload = [[imageProcess alloc]init];
        [upload postRequestWithURL:[httpUrl stringByAppendingString:@"/upload/img"] postParems:nil image:[info objectForKey:UIImagePickerControllerOriginalImage] picFileName:[NSString stringWithFormat:@"%lld",[Util getTimeUtil1970]] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            
            if (error) {
                NSLog(@"============Error:%@",error.localizedDescription);
                [self uploadPicCallback:nil upImage:[info objectForKey:UIImagePickerControllerOriginalImage]];
            }else {
                NSString* result= [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
                NSNumber *code = [[Util StringOrDic2NSDic:result] objectForKey:@"code"];
                if (code.intValue == 0) {
                    [self uploadPicCallback:result upImage:[info objectForKey:UIImagePickerControllerOriginalImage]];
                }else {
                    [self uploadPicCallback:nil upImage:[info objectForKey:UIImagePickerControllerOriginalImage]];
                }
            }
