...
- (void)uploadImageWithimage:(UIImage *)img completionHandler:(void (^)(NSURLResponse *response, id responseObject, NSError *error))completionHandler {
    
    NSString *name = @"pic";
    NSString *fileName = @"";
    NSData *data =  UIImageJPEGRepresentation(img, 0.5);
    if (data) {
        fileName = [NSString stringWithFormat:@"%@.jpg",[NSString stringWithFormat:@"%lld",[Util getTimeUtil1970]]];
    }else {
        data = UIImagePNGRepresentation(img);
        fileName = [NSString stringWithFormat:@"%@.png",[NSString stringWithFormat:@"%lld",[Util getTimeUtil1970]]];
    }
    
    [self uploadImageWithHost:[httpUrl stringByAppendingString:@"/upload/img"]  parameters:nil fileData:data name:name fileName:fileName progress:nil completionHandler:completionHandler];
    
}

- (void)uploadImageWithHost:(NSString *)host parameters:(NSDictionary *)params fileData:(NSData *)data name:(NSString *)name fileName:(NSString *)fileName progress:(void (^)(NSProgress *uploadProgress)) uploadProgressBlock completionHandler:(void (^)(NSURLResponse *response, id responseObject, NSError *error))completionHandler {
    
    NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:host parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
...
-(void)postRequestWithURL: (NSString *)url  // IN
               postParems: (NSMutableDictionary *)postParams // IN
    ...
    //    }
//    [postParams setObject:[NSNumber numberWithLongLong:[Util getTimeUtil1970]] forKey:@"once"];
//    [postParams setObject:[NSNumber numberWithLongLong:[Util getTimeUtil1970]/1000] forKey:@"timestamp"];
//    [postParams setObject:[Nationnal shareNationnal].token forKey:@"token"];
//    
//    NSLog(@"============%@",postParams);
//    
//    NSString * postString = @"";
...
//    [keyArr addObjectsFromArray:[postParams allKeys]];
//    int i = 0;
//    [keyArr sortUsingSelector:@selector(compare:)];
//    for(NSString *key in keyArr) {
//        NSString *val = [Util replaceHttpUrlTag:[postParams objectForKey:key]];
...
//        i++;
//    }
//    [postParams setObject:[Util MD5:[postString stringByAppendingString:secreKey]] forKey:@"sign"];
...
//    NSString *end = @"\r\n";
    ...
    for(int i=0;i<[keys count];i++)
    {
        ...
        NSLog(@"============upload post parameters:%@",[postParams objectForKey:key]);
    }
...
-(void)postRequestWithURL2: (NSString *)url  // IN
    ...
    for(int i=0;i<[keys count];i++)
    {
        ...
        NSLog(@"============upload post parameters:%@",[postParems objectForKey:key]);
    }
