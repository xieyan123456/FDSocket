...
+ (Message *)getBufQSTBody:(NSString *)str type:(NSString*)type packNumber:(NSString*)packNumber isLeave:(BOOL)isLeave{
    ...
    if ([type isEqualToString:@"image"]) {
        ...
        NSMutableDictionary *imageDic = [[Util StringOrDic2NSDic:str] mutableCopy];
        
        NSString *appVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
        [imageDic setObject:appVersion forKey:@"SDK_Version"];
        
        [bodyDic setValue:imageDic forKey:@"image"];
